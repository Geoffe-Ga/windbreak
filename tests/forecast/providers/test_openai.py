"""Tests for windbreak.forecast.providers.openai (issue #191).

Pins `OpenAiChatTransport`: an `LlmTransport` (SPEC S8.9) over the OpenAI
chat-completions HTTP API, mirroring `AnthropicMessagesTransport`'s contract
exactly except for the response envelope's shape. Its request body is a
deterministic, canonical (sorted-key, no-space-separator) JSON object naming
exactly `max_tokens`, `messages` (a single `user` turn carrying the prompt
verbatim), `model` (the request's pinned `model_version`), and an *integer*
`temperature` of `0` -- never a float `0.0`.
...
non-finite JSON constant anywhere in the envelope.
`choices[0]["message"]["content"]` is extracted and returned verbatim as the
completion text; any other envelope shape (not an object, missing/malformed
`choices`, a malformed `message`, a missing/non-string `content`) is rejected
as `RESPONSE_FAILURE_MALFORMED_VOTE_JSON`.

`windbreak.forecast.providers.openai` does not exist yet, so importing it
below fails collection with `ModuleNotFoundError: No module named
'windbreak.forecast.providers.openai'` -- the expected Gate 1 RED state for
issue #191.
"""
...
#: The pinned OpenAI model version exercised by these tests, matching the new
#: #191 default vote-ensemble member's `model_version`.
_MODEL_VERSION = "gpt-5-2025-08-07"

#: An arbitrary, fixed prompt text: `OpenAiChatTransport` only ever relays
#: `LlmRequest.prompt` verbatim, so its own tests never need a real
...
_VALID_VOTE_JSON = (
    '{"probability_ppm": 388888, "rationale_summary": "mixed but leaning no", '
    '"abstain": false}'
)
...
def _envelope_body(text: str = "a completion") -> str:
    """Build a well-formed OpenAI chat-completions response envelope.

    Args:
        text: The completion text embedded at `choices[0].message.content`.
...
def _transport(body: str, *, status_code: int = 200) -> OpenAiChatTransport:
    """Build an `OpenAiChatTransport` over a stub returning `body`.
...
def _request(prompt: str = _PROMPT_TEXT) -> LlmRequest:
    ...
    """
    return LlmRequest(provider="openai", model_version=_MODEL_VERSION, prompt=prompt)
...
def test_complete_happy_path_returns_the_exact_embedded_completion_text() -> None:
    """A clean envelope's `choices[0].message.content` is returned verbatim."""
...
def test_two_calls_with_equal_requests_build_byte_identical_bodies() -> None:
    ...
    transport_a = OpenAiChatTransport(
        stub_a, endpoint_url=_ENDPOINT_URL, max_tokens=_MAX_TOKENS
    )
    transport_b = OpenAiChatTransport(
...
def test_non_finite_json_constant_anywhere_in_envelope_is_rejected() -> None:
    ...
    """
    body = '{"choices": [{"message": {"content": "ok"}}], "usage": {"tokens": NaN}}'
    transport = _transport(body)
...
def test_fixture_vote_provider_over_openai_transport_yields_validated_forecast(
    market: NormalizedMarket, baseline: BaselineQuoteSnapshot
) -> None:
    """`FixtureVoteProvider(OpenAiChatTransport(...), member)` end-to-end: a
    clean, schema-valid completion embedded in a clean envelope yields a
    ...
    member = EnsembleMember("openai", _MODEL_VERSION, "2024-09-30")
    ...
    assert result.probability_ppm == 388_888
    assert result.rationale_summary == "mixed but leaning no"
    assert result.provider == "openai"
    assert result.model_version == _MODEL_VERSION
    assert result.training_cutoff == "2024-09-30"
    assert (
