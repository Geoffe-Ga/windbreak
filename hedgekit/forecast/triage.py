...
(:func:`hedgekit.forecast.pipeline.run_pipeline`). SPEC S8.4 puts a cheap
...
:class:`hedgekit.connector.snapshot.EventLedgerWriter`): a ``TRIAGE_STOP`` or
...
#: ``hedgekit.forecast.pipeline``'s private ``_RESEARCH_COST_MICROS`` stub
...
class _TriageModel(NamedTuple):
    ...
    Mirrors :class:`hedgekit.forecast.pipeline._EnsembleMember`: pinning the
