...
:func:`hedgekit.forecast.triage.run_stage0_prior`'s single-call shape),
...
verbatim on :class:`hedgekit.forecast.triage.TriageLedgerWriter`): a
...
from hedgekit.alerts import AlertType
...
#: It mirrors :data:`hedgekit.forecast.triage.TRIAGE_THRESHOLD_PPM`'s 5-point
...
class _CanaryModel(NamedTuple):
    ...
    Mirrors :class:`hedgekit.forecast.triage._TriageModel`: pinning the
...
class CanaryAlertEmitter(Protocol):
    ...
    A real :class:`hedgekit.alerts.AlertDispatcher` satisfies this structurally.
    """
...
def _parse_observed_ppm(response: str) -> int:
    ...
    Mirrors :func:`hedgekit.forecast.triage._parse_prior_ppm`'s contract with a
