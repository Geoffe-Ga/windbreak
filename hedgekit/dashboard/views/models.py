...
:mod:`hedgekit.ledger.rebuild` produces. An empty bundle is the documented
...
functions ``hedgekit rebuild`` writes so the dashboard never re-derives its own
view of the ledger.
"""
...
#: :mod:`hedgekit.ledger.rebuild` projection emits.
...
def build_ledger_read_models_source(
    ...
    projection functions ``hedgekit rebuild`` writes -- so every request reflects
    live ledger truth and a corrupt ledger fails closed
    (:class:`~hedgekit.ledger.store.ChainIntegrityError`) rather than rendering a
    ...
    Returns:
        ...
        :func:`hedgekit.dashboard.app.create_server`'s ``read_models_source``.
    """
    from hedgekit.ledger.rebuild import (
