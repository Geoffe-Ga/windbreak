...
Gateway's :meth:`~hedgekit.order_gateway.gateway.OrderGateway.recover` and the
:class:`~hedgekit.order_gateway.reconciler.Reconciler` fold the durable ledger,
...
mirroring :mod:`~hedgekit.order_gateway.state_machine`'s data-not-branches style)
keeps every function trivially auditable and well under the complexity ceiling.
    ...
    * :func:`fold_ledger_states` / :func:`ledger_shows_halt` -- fold the durable
      ...
      :class:`~hedgekit.ledger.events.ReduceOnlyViolation` or
      :class:`~hedgekit.ledger.events.ReconciliationHalted` is durable and has no
      un-halt event).
...
from hedgekit.ledger.events import ReconciliationHalted
...
class TrackedOrder:
    ...
    :class:`~hedgekit.connector.models.OpenOrder`/``Fill`` truth.
...
def fold_ledger_states(records: Iterable[LedgerRecord]) -> dict[str, OrderState]:
    ...
    Replays only :class:`~hedgekit.ledger.events.OrderTransitionLedgered` rows,
    ...
    Returns:
        ...
        :class:`~hedgekit.order_gateway.state_machine.OrderState`.
    """
...
def pending_intents(wal_records: Iterable[WalRecord]) -> list[OrderIntent]:
    ...
    Returns:
        The journalled :class:`~hedgekit.riskkernel.checks.OrderIntent`s that
...
def build_unaccounted_halt(
    ...
    Returns:
        The :class:`~hedgekit.ledger.events.ReconciliationHalted` for the first
