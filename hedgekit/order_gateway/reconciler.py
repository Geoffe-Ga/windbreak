...
when in doubt, halt):
    ...
    * a Gateway-placed resting order that filled out-of-band (tracked order gone
      ...
      :class:`~hedgekit.ledger.events.ReconciliationHealed`, and -- for a close
...
:class:`~hedgekit.ledger.events.ReconciliationHalted` and latches the *live*
Gateway halted (stopping ``accepting_approvals``), never silently guessing a
benign fill.

:meth:`Reconciler.run` mirrors :meth:`~hedgekit.order_gateway.gateway.
...
from hedgekit.ledger.events import ReconciliationHalted, ReconciliationHealed
...
if TYPE_CHECKING:
    from hedgekit.connector.models import OpenOrder
...
class Reconciler:
    ...
    def _heal(self, order: TrackedOrder, states: dict[str, OrderState]) -> None:
        ...
        a :class:`~hedgekit.ledger.events.ReconciliationHealed`, and retires the
