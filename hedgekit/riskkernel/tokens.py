...
This is the *sole* repo importer of :mod:`hedgekit.riskkernel.signing` outside
the signing module itself: it pairs the isolated :class:`SigningKeyHandle` with
the shared :func:`~hedgekit.tokens.verify.canonical_claims_bytes` encoding to
mint a :class:`~hedgekit.tokens.verify.SignedApprovalToken`. Verification lives
in the shared :mod:`hedgekit.tokens` package, which never sees the key handle.
"""
...
from hedgekit.tokens.verify import SignedApprovalToken, canonical_claims_bytes
...
class TokenIssuer:
    ...
    def from_key_material(cls, key_material: bytes) -> TokenIssuer:
        ...
        :mod:`hedgekit.riskkernel.signing` itself -- preserving the signing-key
