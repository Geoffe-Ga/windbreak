...
:class:`~hedgekit.config.schema.HedgekitConfig`, filling any absent section
...
from hedgekit.config.schema import _CONVERT, _YAML_KEY, HedgekitConfig
from hedgekit.config.versioning import config_hash, diff_configs

if TYPE_CHECKING:
    from typing import Any, NoReturn

    from hedgekit.config.recorder import ConfigEventRecorder
...
def _record(
    config: HedgekitConfig, source: str, recorder: ConfigEventRecorder | None
) -> None:
    ...
    recorder.record_config_loaded(
        ...
        diff=diff_configs(HedgekitConfig(), config),
        source=source,
    )
...
def load_config(
    ...
    """
    raw = _read_yaml(path)
    config: HedgekitConfig = _build(HedgekitConfig, raw, "")
...
def load_default_config(
    *, recorder: ConfigEventRecorder | None = None
    ...
    Returns:
        The default configuration, identical to ``HedgekitConfig()``.
    """
