"""Typed, immutable configuration schema for hedgekit (SPEC §16).

Every configuration section is a frozen dataclass whose field defaults are
the SPEC §16 example verbatim, so ``HedgekitConfig()`` is a complete, valid,
...
class OpsConfig:
    ...
    state_dir: str = "~/.local/share/hedgekit"
    backup_dir: str = "~/hedgekit-backups"
...
@dataclass(frozen=True, slots=True)
class HedgekitConfig:
    """The complete, immutable hedgekit configuration (SPEC §16 root).
