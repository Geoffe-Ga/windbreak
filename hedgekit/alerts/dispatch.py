...
:class:`~hedgekit.alerts.sinks.LogOnlySink` by default -- fires so an alert is
never silently lost.
...
:class:`LoggingLedgerWriter` that only logs, with no ``hedgekit.ledger``
dependency.
"""
...
from hedgekit.alerts.registry import get_registration
...
if TYPE_CHECKING:
    ...
    from hedgekit.alerts.sinks import AlertSink

_LOGGER = logging.getLogger("hedgekit.alerts")
...
class LoggingLedgerWriter:
    ...
    :class:`LedgerWriter`; it emits on the module ``hedgekit.alerts`` logger.
    """
        ...
        *,
        ...
        Args:
            ...
            fallback: The sink to fire when no primary sink succeeds.
                Defaults to a :class:`~hedgekit.alerts.sinks.LogOnlySink`.
        """
        self._sinks = sinks
