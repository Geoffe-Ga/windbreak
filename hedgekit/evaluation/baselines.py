...
division and therefore no :class:`hedgekit.numeric.rounding.RoundingDirection`
...
This is a leaf module: it imports only :mod:`hedgekit.numeric.types` and is
...
def _quote_snapshot_from_entry(entry: Mapping[str, Any]) -> QuoteSnapshot:
    ...
    Raises:
        TypeError: If a pips field carries a ``bool`` masquerading as an int
            (rejected by :class:`~hedgekit.numeric.types.PricePips`).
...
def _forecast_from_entry(entry: Mapping[str, Any]) -> BaselineForecast:
    ...
    Raises:
        TypeError: If ``probability_ppm`` carries a ``bool`` masquerading as an
            int (rejected by :class:`~hedgekit.numeric.types.ProbabilityPpm`).
    """
