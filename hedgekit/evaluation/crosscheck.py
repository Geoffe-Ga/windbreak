...
(:func:`~hedgekit.evaluation.registry.registered_metrics`, the #51/#53 machinery)
and the SQL path (:class:`~hedgekit.evaluation.sql_gates.SqlGateComputer`) both
...
raised (:data:`~hedgekit.evaluation.sql_gates.SQL_QUERY_FAILED`) or a Python
...
:mod:`hedgekit.evaluation.preregistration`): :class:`GateComputationMismatch`
...
from hedgekit.evaluation.cohorts import UndefinedBrier
from hedgekit.evaluation.registry import (
    NotImplementedSentinel,
    gate_evaluation_inputs,
    registered_metrics,
)
from hedgekit.evaluation.sql_gates import SqlGateComputer, SqlGateFailure
...
if TYPE_CHECKING:
    ...
    from hedgekit.evaluation.registry import EvaluationInputs, MetricSpec, MetricValue
    from hedgekit.evaluation.sql_gates import SqlMetricValue
...
#: (rather than imported from the private copy in :mod:`hedgekit.ledger.events`)
...
class PythonComputeFailure(enum.Enum):
    ...
    :class:`~hedgekit.evaluation.sql_gates.SqlGateFailure`: a reference metric
    ...
    (including :data:`~hedgekit.evaluation.sql_gates.SQL_QUERY_FAILED`), so it
...
def _render_value(
    ...
    Returns:
        ...
        :func:`hedgekit.evaluation.report._format_value`), else the ``int``
        itself.
    """
    if isinstance(
        value,
...
def _python_value(
    ...
    :meth:`~hedgekit.evaluation.sql_gates.SqlGateComputer._value_for`): the
    ...
    *,
    ...
    Args:
        ...
        sql_path: The SQL computer to run; a default
            :class:`~hedgekit.evaluation.sql_gates.SqlGateComputer` when ``None``.
        tolerance: The inclusive absolute tolerance for two ``int`` values.
