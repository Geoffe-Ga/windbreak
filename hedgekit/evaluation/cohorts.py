...
imports :mod:`hedgekit.evaluation.metrics` and
:mod:`hedgekit.evaluation.windows` at runtime and references
:mod:`hedgekit.evaluation.registry` types only under
...
from hedgekit.evaluation.metrics import BASELINE_PPM_PER_PIP, mean_brier
from hedgekit.evaluation.windows import combine, resolve_window

if TYPE_CHECKING:
    from collections.abc import Iterable

    from hedgekit.evaluation.registry import EvaluationInputs, FixtureForecast
    from hedgekit.evaluation.windows import ObservationWindow, WindowedForecasts
...
class UndefinedBrier(enum.Enum):
    ...
    :class:`~hedgekit.evaluation.registry.NotImplementedSentinel`.
    """
...
def _windowed_cohort_forecasts(
    ...
    narrowing to the cohort, :func:`hedgekit.evaluation.windows.resolve_window`
...
def cohort_brier_table(
    ...
    then scored with :func:`hedgekit.evaluation.metrics.mean_brier`. The window
...
def mean_brier_over(
    ...
    Raises:
        MixedObservationWindowError: If the slices name more than one window
            (propagated from :func:`hedgekit.evaluation.windows.combine`).
