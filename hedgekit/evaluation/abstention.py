...
full binary payout (:data:`~hedgekit.evaluation.metrics.PAYOUT_PIPS`) and the
ppm-per-pip conversion (:data:`~hedgekit.evaluation.metrics.BASELINE_PPM_PER_PIP`)
are imported from :mod:`hedgekit.evaluation.metrics` rather than re-declared, and
:class:`~hedgekit.evaluation.resolution.ResolutionOutcome` is the one other
