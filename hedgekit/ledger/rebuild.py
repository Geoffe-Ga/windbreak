...
hash chain, then folds the records in sequence order into two byte-stable
read-model files -- ``config_versions.json`` (the ``ConfigLoaded`` rows)
and ``mode_history.json`` (the ``ModeHeartbeat`` rows). ``AlertEmitted``
and any unrecognized event types are skipped. Because verification runs
first, a corrupt ledger raises :class:`ChainIntegrityError` instead of
producing a plausible-but-wrong projection.
