...
:mod:`windbreak.selector` package is on ``scripts/lint_no_floats.py``'s denylist
(SPEC S6.1). This module imports only :mod:`windbreak.numeric`, never
:mod:`windbreak.selector.types`, so wiring it into ``SelectorInputs`` introduces
no import cycle.
"""
