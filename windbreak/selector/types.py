...
:class:`~windbreak.connector.fees.FeeModel` plus its ``as_of`` freshness stamp),
:class:`SlippageModelInput` (a per-contract ppm buffer), and
:class:`RiskConfigInput` (a real :class:`~windbreak.config.schema.RiskConfig`
...
mirrors the *shape* of :class:`~windbreak.riskkernel.context.AccountState` but
...
issue #47's, and the mode-gated caps stay fenced (see :mod:`windbreak.selector.
sizing`).
...
:class:`~windbreak.numeric.MoneyMicros`, and the remaining arithmetic-bearing
values live inside the already unit-typed
:class:`~windbreak.forecast.records.ForecastRecord`,
:class:`~windbreak.connector.models.OrderBookSnapshot`, and
:class:`~windbreak.connector.fees.FeeModel`.
"""
...
#: :func:`windbreak.selector.execution_style.decide_execution_style` proves the
...
class FeeModelInput:
    ...
    :class:`~windbreak.connector.fees.FeeModel` the executable-edge arithmetic
...
class RiskConfigInput:
    ...
    Carries the real :class:`~windbreak.config.schema.RiskConfig` whose
...
class PositionReadModelInput:
    ...
    :class:`~windbreak.numeric.MoneyMicros` (SPEC S6.1, no floats on the money
    path). The field naming mirrors
    :class:`~windbreak.riskkernel.context.AccountState`, but this type imports
    ...
    Attributes:
        ...
        bucket_exposure: The correlation-bucket exposure the per-bucket cap
            ...
            placeholder: :func:`~windbreak.selector.select` computes the real
...
class SelectorOrderIntent(OrderIntent):
    ...
    :class:`~windbreak.riskkernel.checks.OrderIntent` (IS-A, never a parallel
...
#: :class:`~windbreak.riskkernel.checks.OrderIntent` that *extends it by
...
class SelectorInputs:
    ...
    Attributes:
        ...
        correlation_tags: The target market's own correlation-bucket tags (SPEC
            ...
            :func:`~windbreak.selector.correlation.effective_buckets` (a human
