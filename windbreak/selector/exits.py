...
:func:`windbreak.selector.select`, the open-side entry point. SPEC S6.4 requires
...
It emits a reduce-only :class:`~windbreak.selector.types.SelectorOrderIntent`
...
def _close_idempotency_key(
    ...
    :func:`windbreak.order_gateway.client_order_id.client_order_id` and
    :func:`windbreak.selector.__init__._idempotency_key` use, so the key is a
...
def build_close_intent(
    ...
    Returns:
        The reduce-only :class:`~windbreak.selector.types.SelectorOrderIntent`.
