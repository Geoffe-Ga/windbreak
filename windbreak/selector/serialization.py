...
fixture. This module projects a :class:`~windbreak.selector.types.SelectorDecision`
into a plain, JSON-safe mapping and defers the byte-stable encoding to
:func:`~windbreak.ledger.events.canonical_json` (sorted keys, no whitespace) --
...
:func:`windbreak.forecast.records.forecast_record_to_payload`.
"""
...
if TYPE_CHECKING:
    from windbreak.selector.types import NormalizedOrderIntent, SelectorDecision
...
def _intent_to_payload(intent: NormalizedOrderIntent) -> dict[str, object]:
    ...
    :func:`windbreak.forecast.records.forecast_record_to_payload`. The three
...
def serialize_decision(decision: SelectorDecision) -> str:
    ...
    :class:`~windbreak.selector.types.SelectorDecision` fields -- no generated
    timestamp is ever added -- and delegates the encoding to
    :func:`~windbreak.ledger.events.canonical_json` so the bytes are a stable
