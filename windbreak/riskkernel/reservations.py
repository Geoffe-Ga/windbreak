...
pipeline that turns an approved intent into reserved-and-signed approval token:
    ...
    * :class:`ReservationLedger` -- a single-lock ledger assigning monotonic
      ...
      one :class:`~windbreak.ledger.events.Event` per mutation.
...
Every monetary quantity is a :mod:`windbreak.numeric` scaled integer, never a
...
from windbreak.numeric import RoundingDirection, money_from_price_and_count
from windbreak.numeric.types import MoneyMicros
from windbreak.riskkernel import checks
from windbreak.riskkernel.checks import _order_cost
from windbreak.riskkernel.tokens import DEFAULT_TOKEN_TTL_SECONDS
...
class ReservationLedger:
    ...
    def release_all_active(self, *, reason: str) -> None:
        ...
        :class:`~windbreak.ledger.events.Event` (``ReservationReleased``) per
...
class ApprovalPipeline:
        ...
        *,
        ...
        Args:
            ...
            ttl_seconds: The issued token's lifetime, in seconds. Defaults to
                :data:`~windbreak.riskkernel.tokens.DEFAULT_TOKEN_TTL_SECONDS`.
    ...
    def _reserve_and_issue(
        ...
        the shared :func:`~windbreak.riskkernel.checks._order_cost`, whose
