...
:mod:`windbreak.evaluation.registry` re-exports it, so
...
:class:`~windbreak.evaluation.registry.FixtureForecast` only under
...
if TYPE_CHECKING:
    ...
    from windbreak.evaluation.registry import FixtureForecast
...
class ObservationWindow(enum.Enum):
    ...
    each :class:`~windbreak.evaluation.registry.MetricSpec` and rendered beside
    the metric's value.
    """
...
#: :mod:`windbreak.evaluation.registry` and :mod:`windbreak.evaluation.report` so
