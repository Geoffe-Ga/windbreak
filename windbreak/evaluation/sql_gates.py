...
value that :mod:`windbreak.evaluation.metrics` /
:mod:`windbreak.evaluation.cohorts` produce, but from scratch, in SQLite, sharing
*no* computation with the Python reference path.
:mod:`windbreak.evaluation.crosscheck` runs both paths on every gate evaluation
and alerts loudly when they disagree.

Independence is structural, not merely conventional. This module must NOT import
:mod:`windbreak.evaluation.metrics`, :mod:`windbreak.evaluation.bootstrap`, or
:mod:`windbreak.evaluation.power`, and must NOT use
:func:`windbreak.numeric.rounding.divide`: every division here is a local integer
...
from windbreak.evaluation.registry import NOT_IMPLEMENTED, NotImplementedSentinel
...
#: than imported from :mod:`windbreak.evaluation.metrics` so the two paths share
...
class SqlGateFailure(enum.Enum):
    ...
    :class:`~windbreak.evaluation.crosscheck.MetricComparison` as a loud,
...
#: :data:`~windbreak.evaluation.registry.NOT_IMPLEMENTED` stub sentinel, the
#: :data:`~windbreak.evaluation.cohorts.UNDEFINED` empty-cohort sentinel, or
...
def _local_floor_div(numerator: int, denominator: int) -> int:
    ...
    code with :mod:`windbreak.numeric.rounding` (issue #55 independence rule).
...
class SqlGateComputer:
    ...
    def compute(
        ...
        Returns:
            ...
            :data:`~windbreak.evaluation.registry.NOT_IMPLEMENTED` sentinel for the
            execution-track stub, the :data:`~windbreak.evaluation.cohorts.UNDEFINED`
    ...
    def _value_for(
        ...
        Returns:
            ...
            :data:`~windbreak.evaluation.registry.NOT_IMPLEMENTED` for the stub,
            :data:`~windbreak.evaluation.cohorts.UNDEFINED` for a ``NULL``
