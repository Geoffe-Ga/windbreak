...
This module wires the Gateway's runtime surface:
    ...
    * :class:`OrderSubmitter`: the structural protocol the Gateway submits
      ...
      :class:`~windbreak.riskkernel.checks.OrderIntent` to the paper exchange's
      :class:`~windbreak.connector.paper.PaperOrderIntent`.
    * :class:`OrderGateway`: verifies each single-use approval token *before*
      submitting, walking the real :mod:`~windbreak.order_gateway.state_machine`
      ...
      *brand-new* intent when the exchange is not ``"open"`` (before verifying,
      ...
      :func:`~windbreak.order_gateway.client_order_id.client_order_id` per intent
      ...
      *before* taking the next action. An *already-acked* intent replays its
      ...
      an :class:`~windbreak.riskkernel.checks.OrderIntent` (and the paper
      exchange's :class:`~windbreak.connector.paper.PaperOrderIntent`) always
      carries a :class:`~windbreak.numeric.PricePips` price -- there is no
...
from windbreak.ledger.events import RecoveryCompleted
...
from windbreak.order_gateway.sweeper import RestingOrderMeta
from windbreak.order_gateway.tokens import VerifyResult, verify_and_consume
...
if TYPE_CHECKING:
    ...
    from windbreak.connector.models import ExchangeStatus, OpenOrder, Position
    ...
    from windbreak.order_gateway.sweeper import Sweeper, SweepOutcome
    ...
    from windbreak.tokens.verify import SignedApprovalToken, SingleUseRegistry
...
#: ``windbreak.riskkernel.signing.SigningKeyHandle.from_env`` signs under -- the
...
_LOGGER = logging.getLogger("windbreak.order_gateway")
...
def _outcome_to_side(outcome: str) -> Literal["yes", "no"]:
    ...
    Returns:
        ``"yes"`` or ``"no"``, the :class:`~windbreak.connector.paper.PaperOrderIntent`
        side.
...
class PaperSubmitter:
    ...
    Adapts an :class:`~windbreak.riskkernel.checks.OrderIntent` into a
    :class:`~windbreak.connector.paper.PaperOrderIntent`, places it, and projects
    the resulting :class:`~windbreak.connector.paper.PaperPlacement` into a
    :class:`SubmissionAck`.
    """
...
class GatewayStatusSource(Protocol):
    ...
    def get_exchange_status(self) -> ExchangeStatus | None:
        ...
        Returns:
            The current :class:`~windbreak.connector.models.ExchangeStatus`, or
...
class GatewayPositionSource(Protocol):
    ...
    only for a *closing* intent; a :class:`~windbreak.connector.models.Position`
    tuple feeds the reduce-only admission math in
    :mod:`windbreak.order_gateway.reduce_only`.

    Timing contract (load-bearing, see
    :func:`~windbreak.order_gateway.reduce_only.is_net_short_after_fill`): the
...
class GatewayHaltedError(Exception):
    ...
    :class:`~windbreak.order_gateway.ledger_writer.ReduceOnlyViolation`, latches a
...
class _UnwiredSubmitter:
    ...
    mirroring how :mod:`windbreak.riskkernel.process`'s ``main`` wires a
...
class OrderGateway:
    ...
    :class:`~windbreak.riskkernel.signing.SigningKeyHandle`'s no-leak guarantee
    ...
    :func:`~windbreak.order_gateway.client_order_id.client_order_id` is the
        ...
        *,
        ...
        Args:
            ...
            registry: The single-use registry gating replay. Defaults to a fresh
                :class:`~windbreak.tokens.verify.InMemorySingleUseRegistry`.
    ...
    def process_intent(
        ...
        1. A *brand-new* intent (one whose
           :func:`~windbreak.order_gateway.client_order_id.client_order_id` is
    ...
    def recover(self) -> RecoveryReport:
        ...
        ledgers a :class:`~windbreak.ledger.events.RecoveryCompleted` and flips
        ...
        Returns:
            The :class:`~windbreak.order_gateway.recovery.RecoveryReport` summary.
        """
    ...
    def tracked_orders(self) -> tuple[TrackedOrder, ...]:
        ...
        Returns:
            ...
            the unit the :class:`~windbreak.order_gateway.reconciler.Reconciler`
    ...
    def resting_meta(self, order_id: str) -> RestingOrderMeta | None:
        ...
        Returns:
            The order's :class:`~windbreak.order_gateway.sweeper.RestingOrderMeta`
    ...
    def attach_sweeper(self, sweeper: Sweeper) -> None:
        """Attach the :class:`~windbreak.order_gateway.sweeper.Sweeper` (issue #41).
    ...
    def sweep(self) -> SweepOutcome:
        ...
        Returns:
            The attached Sweeper's :class:`~windbreak.order_gateway.sweeper.
...
def build_parser() -> argparse.ArgumentParser:
    """Build the ``windbreak.order_gateway`` bounded-heartbeat CLI parser.

    Both options parse as non-negative integers (a negative value is an argparse
    usage error, exit code 2), matching :mod:`windbreak.riskkernel.process`'s
