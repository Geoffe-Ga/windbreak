...
seam and re-exports the event types that flow through it:
    ...
    * :class:`GatewayLedgerWriter` -- the structural protocol the Gateway
      ...
      every event to a real :class:`~windbreak.ledger.store.SqliteLedgerStore`,
      mirroring the Risk Kernel writer triad in
      :mod:`windbreak.riskkernel.process`.
    * :class:`OrderTransitionLedgered` / :class:`SubmissionRefused` /
      :class:`ReduceOnlyRefused` / :class:`ReduceOnlyViolation` -- re-exported
      from :mod:`windbreak.ledger.events`, which owns the canonical event-schema
...
from windbreak.ledger.events import OrderTransitionLedgered as OrderTransitionLedgered
from windbreak.ledger.events import ReduceOnlyRefused as ReduceOnlyRefused
from windbreak.ledger.events import ReduceOnlyViolation as ReduceOnlyViolation
from windbreak.ledger.events import SubmissionRefused as SubmissionRefused
from windbreak.order_gateway.state_machine import transition
...
class LoggingGatewayLedgerWriter:
    ...
    ``windbreak.order_gateway`` logger with the event type in the message so
...
class SqliteGatewayLedgerWriter:
    ...
    :class:`~windbreak.ledger.store.SqliteLedgerStore` (or any
    :class:`~windbreak.ledger.store.LedgerStore`), so the Gateway's lifecycle and
    recovery events become durable, tamper-evident records that a restarted
    Gateway's :meth:`~windbreak.order_gateway.gateway.OrderGateway.recover` can
    fold back (issue #40).
    """
