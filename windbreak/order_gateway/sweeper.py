...
mirrors the :class:`~windbreak.order_gateway.reconciler.Reconciler`'s discipline:
...
Two kinds of staleness drive a cancel:
    ...
    * **Move breach** -- ``abs(observed - baseline) > move_ticks *
      ...
      :class:`~windbreak.ledger.events.MarketFreeze` is ledgered before the
      ticker's cancels and one :class:`~windbreak.ledger.events.ReturnToScreener`
...
:meth:`Sweeper.run` mirrors :meth:`~windbreak.order_gateway.reconciler.
...
from windbreak.ledger.events import MarketFreeze, ReturnToScreener
...
if TYPE_CHECKING:
    ...
    from windbreak.connector.models import Fill, NormalizedMarket, OrderBookSnapshot
...
#: mirroring :data:`~windbreak.order_gateway.reconciler._EPOCH`.
...
class Sweeper:
    ...
    def run(
        ...
        *,
        ...
        Mirrors :meth:`~windbreak.order_gateway.reconciler.Reconciler.run`: the
    ...
    def _freeze_ticker(
        ...
        Ledgers exactly one :class:`~windbreak.ledger.events.MarketFreeze` *before*
        the ticker's cancels and exactly one
        :class:`~windbreak.ledger.events.ReturnToScreener` after they resolve, and
