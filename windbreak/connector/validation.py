...
:func:`windbreak.connector.kalshi.normalize.payload_hash` (kept in lock-step here
...
injected wall clock through :func:`windbreak.connector.snapshot.utc_now_iso`.
"""
...
from windbreak.connector.snapshot import ConnectorEvent, utc_now_iso
...
_LOGGER = logging.getLogger("windbreak.connector.validation")


def _payload_fingerprint(payload: Mapping[str, object]) -> str:
    """Return the canonical-JSON SHA-256 fingerprint of a full payload.

    Mirrors :func:`windbreak.connector.kalshi.normalize.payload_hash`: the digest
...
def kalshi_default_schema_registry() -> SchemaRegistry:
    ...
    fields :mod:`windbreak.connector.kalshi.normalize` consumes. The order book's
