...
You are a senior Python engineer building fail-safe control paths, working in this repo's `hedgekit/riskkernel/` package.

## Goal

Every SPEC §10.11 kill trigger — dashboard button (double-confirm), CLI `hedgekit kill`, a `KILL` file in the state dir (works with HTTP down), and automatic on repeated reconciliation mismatch — cancels all open orders, disables approvals, alerts, holds positions, and requires manual typed-confirmation re-arm.
...
- [ ] `hedgekit rearm`: requires typing an exact confirmation phrase including the kill event's ledger sequence number; re-arm transitions `KILLED → PAUSED` (operator then resumes explicitly), ledgered
...
**Tracer-code invariant:** The system must remain demoable after this PR merges: normal flow untouched until a trigger fires; after kill, the system is visibly and safely dead until `hedgekit rearm`.
