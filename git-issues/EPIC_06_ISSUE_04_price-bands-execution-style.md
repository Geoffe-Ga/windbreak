...
You are a senior Python engineer with market-microstructure experience, working in this repo's `windbreak/selector/` package on adverse-selection defenses.
...
- **Files involved:**
  - `windbreak/selector/execution_style.py` — cross vs. rest decision, TTL/cancel-on-move stamping (new)
  - `windbreak/selector/entry.py` — replace the price-band stub from #44 with real enforcement
  - `windbreak/selector/exits.py` — close-intent construction gated to the three legitimate trigger sources, reduce-only always (new)
...
**Scope fence:** Do not implement the Gateway-side sweeper (cancel on TTL/move — that is EPIC_05's scope), strategy-driven early exits (post-v1, §19), or correlation buckets (#47). If you find yourself touching `windbreak/order_gateway/`, stop.
