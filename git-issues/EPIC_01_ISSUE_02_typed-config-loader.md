...
- **Predecessor issue(s):** #EPIC_01_ISSUE_01_NUMBER (must be merged first — package layout and CLI exist).
...
- **Prior decisions:** integer units only — ppm/micros/pips fields are `int`; no float fields anywhere in config (§6.1). Unknown keys are fatal, not warnings (§16 heading). Ledgering the version is a hook: emit a `ConfigLoaded(config_hash, diff)` event via a narrow interface that issue #EPIC_01_ISSUE_04_NUMBER will back with the real ledger; land it here as an in-memory recorder protocol.
...
**Scope fence:** Do not implement floor-lowering governance, cool-offs, or mode transitions (EPIC_04 / Risk Kernel), and do not write to a real ledger (issue #EPIC_01_ISSUE_04_NUMBER). Do not add config keys absent from §16. If you find yourself touching files outside the list above, stop and check with the user.
