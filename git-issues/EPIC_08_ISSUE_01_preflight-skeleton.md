...
You are a senior Python engineer specializing in credential hygiene and fail-closed operational tooling, working in this repo's `windbreak/` package (Python ≥3.11, mypy --strict).

## Goal

`windbreak preflight` runs a typed checklist of production-readiness checks — exchange read-only reachability, trade-key scope validation, jurisdiction eligibility, and secrets hygiene — and exits non-zero if any check is not `PASS`, with every check individually fixture-tested for both pass and fail paths.
...
- **Files involved:**
  - `windbreak/preflight/__init__.py` — new package: check definitions + runner (create).
  - `windbreak/preflight/checks.py` — individual check implementations returning a typed result (create).
  - `windbreak/main.py` / the CLI entrypoint — register the `preflight` subcommand.
...
- **State of the world:** connector (EPIC_02), Risk Kernel (EPIC_04), Gateway (EPIC_05), and evaluation (EPIC_07) exist and run in PAPER. Nothing validates production readiness yet; `windbreak preflight` does not exist.

## Output Format

Deliverable is a single PR containing:

- [ ] `windbreak/preflight/` with a `PreflightReport` model: an ordered list of `PreflightCheck {check_id, description, status: PASS|FAIL|SKIP, detail, spec_ref}` — no free-form-only output.
...
- [ ] CLI wiring: `windbreak preflight [--json]`, exit 0 only if every non-SKIP check is PASS.
...
**Tracer-code invariant:** The system must remain demoable after this PR merges. `windbreak run` in PAPER mode must behave exactly as before; preflight is additive. If your change breaks an unrelated CLI surface, you have gone outside scope — revert and re-plan.
