...
You are a senior Python engineer working in this repo's `windbreak/connector/` subpackage, experienced in resilient API clients (rate limiting, backoff, circuit breakers) and fail-closed system design.
...
- **Files involved:**
  - `windbreak/connector/validation.py` — versioned response schemas; unknown-field policy (unknown money/risk field → ledgered `SCHEMA_ANOMALY` + halt signal; unknown cosmetic field → warn)
  - `windbreak/connector/freshness.py` — `fetched_at` stamping and TTL check helpers for consumers
  - `windbreak/connector/resilience.py` — token bucket, backoff with jitter, circuit breaker
  - `windbreak/connector/kalshi/client.py` — wire resilience in; suspend on maintenance windows from `get_exchange_status()`
