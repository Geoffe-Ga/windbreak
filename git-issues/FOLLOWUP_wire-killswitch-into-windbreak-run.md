# feat(main): wire RiskKernel + KillIntegration/KillFileWatcher into `windbreak run`
...
four triggers is composed into the live `windbreak run` process**, so
`windbreak kill` / `windbreak rearm` are inert against a running deployment today.

The Claude reviewer traced every production construction site:

- `windbreak/main.py`'s `_run_heartbeat` / `run_loop` (the only console script
  ...
  from `windbreak.riskkernel.kill`, never `KillSwitch` / `KillIntegration` /
  `KillFileWatcher`.
- `windbreak/riskkernel/process.py` `main()` is the only place a `RiskKernel(` is
  constructed outside tests, and it does **not** pass `kill_integration=`.
- `grep -rn "kill_integration=" windbreak/ tests/` turns up exactly two hits,
  both inside `tests/riskkernel/test_kill.py`.

Concretely: `windbreak kill --state-dir DIR` writes a `KILL` file that no running
...
- [ ] `windbreak kill --state-dir DIR` demonstrably halts the running kernel
      (approvals vetoed) end-to-end.
- [ ] The `windbreak/riskkernel/kill.py` module docstring's "wiring status" note
