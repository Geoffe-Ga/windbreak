...
You are a senior Python engineer with quantitative-modeling experience (probabilistic aggregation, integer fixed-point math), working in this repo's `hedgekit/forecast/` package.
...
- **Files involved:**
  ...
  - `hedgekit/forecast/coherence.py` — group normalization + tolerance flagging (new)
  - `hedgekit/forecast/records.py` — enforce `coherence_flag=True` ⇒ `eligible_for_live=False` (modify)
  - `hedgekit/forecast/pipeline.py` — replace stub vote/aggregation/coherence stages (modify)
...
- [ ] Production code in `hedgekit/forecast/{ensemble,coherence}.py` + pipeline wiring
